...
def buildSegments(bm,a,b,maxDistance,isclosed, kkk):
    print("buildSegments")
    ...
    if kkk:        
        lastvertex=bm.verts.new((kkk.x,kkk.y,kkk.z))
    else:
    ...
    '''
    if othersplinevertex!=None and isclosed:
        bm.edges.new((othersplinevertex,lastvertex))
'''
    while t < 1:
            ...
            for bpIdx in range(thelen):
                ...
                if bpIdx==thelen-1:
                    if spline.use_cyclic_u:
                        lastvertex=buildSegments(bm,spline.bezier_points[bpIdx],spline.bezier_points[0],maxDistance,spline.use_cyclic_u, lastvertex)                        
                else:
